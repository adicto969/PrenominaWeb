...
if($numCol >= 1){
  ...
  while ($row = $objBDSQL->obtenResult()) {
    ...
    foreach ($row as $key => $value) {
      ...
      $fechaN = $date;
      $consultaVV = "SELECT Valor FROM TiempoExtra WHERE Codigo = '".$row["codigo"]."' AND Fecha = '$fechaN' AND Periodo = '$PC' AND Tn = '$TN' AND IDEmpresa = '$IDEmpresa' AND $ComSql2;";
      ...
      if($key != 'Sueldo' && $key != 'Tpo') {
        if($key == 'codigo' || $key == 'Nombre'){
          $objPHPExcel->getActiveSheet()->SetCellValue($letra2.$FILA, $value);
            ...
            $objPHPExcel->getActiveSheet()->SetCellValue($letra2.$FILA, $valorC);
        }
        $letra2++;
      }
    }
...
$objWriter->save(Unidad.'E'.$IDEmpresa.'\\'.'quincenal\excel\pantalla-'.$tipo.'.xls');
echo "1";
