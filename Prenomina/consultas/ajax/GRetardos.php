      ...
      if($row["Tpo"] == "E"){
        ...
        if(!empty($row[$value])){
          ...
          if(empty($array[0][0]))
          {
            ...
            ////////////////////////////////////////////////////////////////////////////////////////////////////////
            /////////////////////////////////////////INSERTAR DATO///////////////////////////////////////////////
            $ConsulPremio = "SELECT valor FROM retardo WHERE codigo ='".$row["codigo"]."' AND fecha = '".$fechaTitulo."' AND TN = '".$Tn."' AND periodo = '".$Periodo."' and IDEmpresa = '".$IDEmpresa."' and ".$ComSql;
            
            $numr = $objBDSQL2->obtenfilas($ConsulPremio);

            if($numr <= 0){

              $Minsert = "INSERT INTO retardo VALUES ('".$row["codigo"]."', '".$fechaTitulo."', '".$array[0][0]."', '".$Tn."', '".$centro."', '".$Periodo."', '".$IDEmpresa."');";

              $objBDSQL2->consultaBD2($Minsert);

            }else {

              $InsertarPT = "UPDATE retardo SET valor = '".$array[0][0]."' WHERE TN = '".$Tn."' AND periodo = '".$Periodo."' AND ".$ComSql." AND IDEmpresa = '".$IDEmpresa."' AND codigo = '".$row["codigo"]."' AND fecha = '".$fechaTitulo."';";

              $objBDSQL2->consultaBD2($InsertarPT);
