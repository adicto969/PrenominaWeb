...
class PDF extends FPDF
{
  ...
  function tabla($sql, $conSql, $Nresul, $NombreEmp, $RFC, $RegisEmp, $periodo, $f1, $f2, $f3, $f4, $cabecera1, $cabecera2, $NomDep, $Ncabecera, $TNom, $IDEmpresa, $LCentro, $bdM, $conSql2)
  {
    ...
    for($i=0; $i<$Ncabecera; $i++){
        ...
        $this->Cell(15,6,$cabecera1[0][$i],1,0,'C');
    }
    $this->Ln();
    ...
    for($i=0; $i<$Ncabecera; $i++){
        $this->Cell(15,6,substr($cabecera2[0][$i], 0, 5),1,0,'C', true);
    }
    ...
    while ($row=$conSql->obtenResult())
    {
      ...
      foreach (array_keys($row) as $value) {
        if($value == 'codigo' ||	$value == 'Nombre' ||	$value == 'Sueldo' ||	$value == 'Tpo'){
          ...
          $CONSULTAM = "SELECT
                      (SELECT valor FROM datos WHERE codigo = '".$row['codigo']."' AND nombre = '".$fechaTitulo."' AND periodoP = '".$periodo."' AND tipoN = '".$TNom."' AND IDEmpresa = '".$IDEmpresa."' AND ".$LCentro.") AS 'A',
                      (SELECT valor FROM datosanti WHERE codigo = '".$row['codigo']."' AND nombre = 'fecha".$fechaTitulo."' AND periodoP = '".$periodo."' AND tipoN = '".$TNom."' AND IDEmpresa = '".$IDEmpresa."' AND ".$LCentro." AND Autorizo1 = 1) AS 'B',
                      (SELECT codigo FROM deslaborado WHERE fecha = '".$fechaTitulo."' AND periodo = '".$periodo."' AND tipoN = '".$TNom."' AND IDEmpresa = '".$IDEmpresa."' AND codigo = '".$row['codigo']."' AND valor = 1 AND ".$LCentro.") AS 'C',
                      (SELECT codigo FROM dobturno WHERE fecha = '".$fechaTitulo."' AND periodo = '".$periodo."' AND tipoN = '".$TNom."' AND IDEmpresa = '".$IDEmpresa."' AND codigo = '".$row['codigo']."' AND valor = 1 AND ".$LCentro.") AS 'D'";

          $consultMedi = $conSql2->consultaBD2($CONSULTAM);

          if($consultMedi === false){
          ...
          $this->Cell(15, 6, $datos, 1, 0, 'C', true);
          $this->SetFillColor(255, 255, 255);
  ...
  function Header()
  {
    ...
    if($GLOBALS['pagina'] == 1 && $Nresul > 20){
      ...
      for($i=0; $i<$Ncap; $i++){
          $this->Cell(15,6,$cabecera[0][$i],1,0,'C');
      }
      $this->Ln();
      ...
      for($i=0; $i<$Ncap; $i++){
          $this->Cell(15,6,substr($cabeceraD[0][$i], 0, 5),1,0,'C', true);
      }
      $this->Cell(10, 6, 'PP', 1, 0, 'C', true);
      $this->Cell(10, 6, 'PA', 1, 0, 'C', true);
      $this->Cell(35, 6, 'FIRMA', 1, 0, 'C', true);
...
while ( $row = $objBDSQL->obtenResult() )
{
  ...
  $CONSULTA = "SELECT nombre, valor FROM datos WHERE codigo = '".$row['codigo']."' AND periodoP = '$periodo' AND tipoN = '$tipoNom' AND IDEmpresa = '$IDEmpresa' AND ".$LCentro.";";
...
$consultaDL = "SELECT codigo, fecha FROM deslaborado WHERE periodo = ".$periodo." AND tipoN = ".$tipoNom." AND IDEmpresa = ".$IDEmpresa." AND valor = 1 AND ".$LCentro;
$query = $bdM->query($consultaDL);
$objBDSQL->consultaBD($consultaDL);
$FILA2 = 1;
while($datosM = $objBDSQL->obtenResult()){

  $querysueldo = "SELECT sueldo FROM empleados WHERE codigo = ".$datosM[0];
  $Sueldo = "";
  $objBDSQL2->consultaBD2($querysueldo);
...
$ConsultaDT = "SELECT codigo, fecha FROM dobturno WHERE periodo = ".$periodo." AND tipoN = ".$tipoNom." AND IDEmpresa = ".$IDEmpresa." AND valor = 1 AND ".$LCentro;
$objBDSQL->consultaBD($ConsultaDT);
while ($datosM = $objBDSQL->obtenResult()) {
  $querysueldo2 = "SELECT sueldo FROM empleados WHERE codigo = ".$datosM[0];
  $Sueldo = "";
  $objBDSQL2->consultaBD2($querysueldo2);
  $datos = $objBDSQL2->obtenResult2();
  $Sueldo = $datos['sueldo'];
  $objBDSQL2->liberarC2();
  $fcM = str_replace('-', '/', $datosM['fecha']);
