...
if(isset($_SESSION['IDUser'])){
  ...
  $consulta = "SELECT Descripcion, Link, ID FROM notificaciones WHERE IdUserDes = ".$_SESSION['IDUser']." AND Estatus = 0 LIMIT 20;";
  ...
  if(Autoriza1 == $_SESSION['User']){
    $consultaPermisos = "SELECT codigo, nombre, valor, ID FROM datosanti WHERE $comSql AND IDEmpresa = $IDEmpresa AND tipoN = $TN AND periodoP = $PC AND Autorizo1 = 0";

    $query1 = $bdM->query($consultaPermisos);
    $numr = 0;
    while ($bdM->recorrer($query1)) {
      $numr++;
    }
    if($numr > 0){
    ...
    $consultaPermisos = "SELECT codigo, nombre, valor, ID FROM datosanti WHERE $comSql AND IDEmpresa = $IDEmpresa AND tipoN = $TN AND periodoP = $PC AND Autorizo2 = 0";
